...
species_paired <- names(sort(taxa_sums(NC_Shark_prey_species_merge.ps_paired.1), decreasing=TRUE))[0:22]
#RRA (OTUs = reads)
NC_Shark_prey_species_merge.ps_paired.1_RRA <- transform_sample_counts(NC_Shark_prey_species_merge.ps_paired.1, function(OTU) OTU/sum(OTU))
NC_Shark_prey_species_merge.ps_paired.1_RRA <- prune_taxa(species_paired, NC_Shark_prey_species_merge.ps_paired.1_RRA)
```

```{r}
###Final graph that compare fecal vs stomach by individual (species)
 plot_bar(NC_Shark_prey_species_merge.ps_paired.1_RRA,x="sample_id", fill="species") + 
...
NC_Shark_prey_species_merge.ps_paired.1.PA.AtlanticSharpnose<-subset_samples(NC_Shark_prey_species_merge.ps_paired.1.PA,Species..scientific.name.=="Rhizoprionodon terraenovae")


#Put it into dataframes
otu_df<-cbind(data.frame(otu_table(NC_Shark_prey_species_merge.ps_paired.1.PA.AtlanticSharpnose)))
...
sample_df<-cbind(data.frame(sample_data(NC_Shark_prey_species_merge.ps_paired.1.PA.AtlanticSharpnose)))
sample_df<-rownames_to_column(sample_df, var = "sample")
sample_material_key<-sample_df%>%
  select(sample,material)

taxa_df<-cbind(data.frame(tax_table(NC_Shark_prey_species_merge.ps_paired.1.PA.AtlanticSharpnose)))
...
#plot 
NC_Shark_prey_species_merge.ps_paired.1.PA.AtlanticSharpnose_plot <- ggplot(merge_material_counts,fill="species") +
...
NC_Shark_prey_species_merge.ps_paired.1.PA.Bonnethead<-subset_samples(NC_Shark_prey_species_merge.ps_paired.1,Species..scientific.name.=="Sphyrna tiburo")

#Put it into dataframes
otu_df<-cbind(data.frame(otu_table(NC_Shark_prey_species_merge.ps_paired.1.PA.Bonnethead)))
otu_df$otu<-rownames(otu_df)
...
sample_df<-cbind(data.frame(sample_data(NC_Shark_prey_species_merge.ps_paired.1.PA.Bonnethead)))
sample_df<-rownames_to_column(sample_df, var = "sample")

sample_material_key<-sample_df%>%
  select(sample,material)

taxa_df<-cbind(data.frame(tax_table(NC_Shark_prey_species_merge.ps_paired.1.PA.Bonnethead)))
taxa_df<-rownames_to_column(taxa_df,var="otu")
otu_species_key<-taxa_df%>%
  select(otu,family,genus,species)
...
#plot 
NC_Shark_prey_species_merge.ps_paired.1.PA.Bonnethead_plot <- ggplot(merge_material_counts,fill="species")+
  geom_bar(aes(x=material, y=rel_prct, color=species, fill=species), stat="identity", position="stack")+
  ylab("Relative Percent of Occurrence by material type") + 
  xlab("Bonnethead") + 
  theme_classic() + theme(text = element_text(size =11)) +
...
NC_Shark_prey_species_merge.ps_paired.1.PA.Bonnethead_plot
...
NC_Shark_prey_species_merge.ps_paired.1.PA.Blacknose<-subset_samples(NC_Shark_prey_species_merge.ps_paired.1.PA,Species..scientific.name.=="Carcharhinus acronotus")

#Put it into dataframes
otu_df<-cbind(data.frame(otu_table(NC_Shark_prey_species_merge.ps_paired.1.PA.Blacknose)))
otu_df$otu<-rownames(otu_df)
...
sample_df<-cbind(data.frame(sample_data(NC_Shark_prey_species_merge.ps_paired.1.PA.Blacknose)))
sample_df<-rownames_to_column(sample_df, var = "sample")

sample_material_key<-sample_df%>%
  select(sample,material)

taxa_df<-cbind(data.frame(tax_table(NC_Shark_prey_species_merge.ps_paired.1.PA.Blacknose)))
taxa_df<-rownames_to_column(taxa_df,var="otu")
otu_species_key<-taxa_df%>%
  select(otu,family,genus,species)
...
#plot 
NC_Shark_prey_species_merge.ps_paired.1.PA.Blacknose_plot <- ggplot(merge_material_counts,fill="species")+
  geom_bar(aes(x=material, y=rel_prct, color=species, fill=species), stat="identity", position="stack")+
  ylab("Relative Percent of Occurrence by material type") + 
  xlab("Blacknose") + 
  theme_classic() + theme(text = element_text(size =11)) +
...
NC_Shark_prey_species_merge.ps_paired.1.PA.Blacknose_plot
...
NC_Shark_prey_species_merge.ps_paired.1.PA.Blacktip<-subset_samples(NC_Shark_prey_species_merge.ps_paired.1.PA,Species..scientific.name.=="Carcharhinus limbatus")

#Put it into dataframes
otu_df<-cbind(data.frame(otu_table(NC_Shark_prey_species_merge.ps_paired.1.PA.Blacktip)))
otu_df$otu<-rownames(otu_df)
...
sample_df<-cbind(data.frame(sample_data(NC_Shark_prey_species_merge.ps_paired.1.PA.Blacktip)))
sample_df<-rownames_to_column(sample_df, var = "sample")
...
taxa_df<-cbind(data.frame(tax_table(NC_Shark_prey_species_merge.ps_paired.1.PA.Blacktip)))
taxa_df<-rownames_to_column(taxa_df,var="otu")
otu_species_key<-taxa_df%>%
  select(otu,family,genus,species)
...
#plot 
NC_Shark_prey_species_merge.ps_paired.1.PA.Blacktip_plot <- ggplot(merge_material_counts,fill="species")+
  geom_bar(aes(x=material, y=rel_prct, color=species, fill=species), stat="identity", position="stack")+
  ylab("Relative Percent of Occurrence by material type") + 
  xlab("Blacktip") + 
  theme_classic() + theme(text = element_text(size =11)) +
...
NC_Shark_prey_species_merge.ps_paired.1.PA.Blacktip_plot

#########################################################################################

#Plot all together
library(gridExtra)
grid.arrange(NC_Shark_prey_species_merge.ps_paired.1.PA.Blacktip_plot, NC_Shark_prey_species_merge.ps_paired.1.PA.Blacknose_plot, NC_Shark_prey_species_merge.ps_paired.1.PA.Bonnethead_plot, NC_Shark_prey_species_merge.ps_paired.1.PA.AtlanticSharpnose_plot, ncol=2)
```
...
pseq_paired %>%
  ...
  ggplot2::stat_ellipse(
    ggplot2::aes(
      colour = Species..scientific.name., 
      group = interaction(Species..scientific.name., material) # Ensures separate ellipses
    )
  )
...
pseq_paired %>%
  ...
  ord_plot(
    axes = c(1, 2),
    colour = "material",  # Color points by material type
    fill = "material",
    shape = "material",
    alpha = 0.5,
    size = 2
  ) +
  geom_line(aes(x = NMDS1, y = NMDS2, group = sample_id), colour = "grey") +  # Black connecting lines
  ggplot2::stat_ellipse(
    ggplot2::aes(
      colour = material, 
      group = interaction(Species..scientific.name., material) # Separate ellipses for species & material
    )
  ) +
  facet_wrap(~Species..scientific.name.) +  # Separate plots per species
  scale_color_manual(values = c("fecal" = "skyblue", "stomach" = "orange")) +  # Custom colors
  scale_fill_manual(values = c("fecal" = "skyblue", "stomach" = "orange")) +   # Custom fill colors
  theme(
    panel.grid = element_blank(),
    panel.grid.major = element_blank()
  )
